...
function validate<T>(contract: string, validator: ValidateFunction, value: unknown): T {
  if (!validator(value)) {
...
export function parseAgentInvocation(value: unknown): AgentInvocationV1 {
  return validate<AgentInvocationV1>('AgentInvocationV1', invocationValidator, value);
}

export function parseAgentResult(value: unknown): AgentResultV1 {
  const result = validate<AgentResultV1>('AgentResultV1', resultValidator, value);
  ...
  if (invalidSucceededResult || invalidFailedResult) {
    throw new ContractValidationError('AgentResultV1', [
      {
        path: '/error',
        message: result.status === 'succeeded' ? 'must be absent when status is succeeded' : 'is required',
        keyword: 'semantic',
      },
    ]);
  }
...
export function parseHttpAgentRunRequest(value: unknown): HttpAgentRunRequestV1 {
  return validate<HttpAgentRunRequestV1>('HttpAgentRunRequestV1', httpRunRequestValidator, value);
}

export function parseHttpAgentRunAccepted(value: unknown): HttpAgentRunAcceptedV1 {
  return validate<HttpAgentRunAcceptedV1>('HttpAgentRunAcceptedV1', httpRunAcceptedValidator, value);
}
