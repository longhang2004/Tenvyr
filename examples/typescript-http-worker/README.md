...
This runnable `echo-analyzer` shows a framework-free AgentWeave Worker with a custom input
...
pnpm --filter @agentweave/example-typescript-http-worker build
...
secret manager, keep `AGENTWEAVE_ALLOW_INSECURE_HTTP=false`, and send SIGTERM for graceful
shutdown.
