...
By building a local graph of `Tenvyr`'s files and symbols, agents can query CodeGraph via SQL or JSON parameters to immediately resolve:
...
To initialize CodeGraph index for the Tenvyr monorepo, run:

```bash
pnpm codegraph:init
```

This executes the helper script [codegraph-init.sh](../scripts/codegraph-init.sh), which creates/updates the index database in `.codegraph/codegraph.db`.
...
_Example Query:_ Find where `ExecutionEngineService` is defined:
