...
To run commands through compression, you can use the monorepo helper script [rtk-compress.sh](file:///Users/longhang/personal_repos/AgentWeave/scripts/rtk-compress.sh). It accepts any shell command and pipes its output through RTK (or a lightweight fallback filter if RTK is not installed locally).
...
function fetchAndCompressLogs(containerName: string): string {
  ...
  return execSync(`./scripts/rtk-compress.sh docker logs ${containerName} --tail 200`).toString();
}
```
