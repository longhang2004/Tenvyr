...
*   **Rules:** Keep `steps.<stepId>.result.<field>` template semantics stable, include `attempt` in task/result events for retry safety, and update `ORCHESTRATOR_AGENT_NAMES` when adding Kafka-backed agents.
