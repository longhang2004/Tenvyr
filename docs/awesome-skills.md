...
## 🚀 Why Use Awesome Skills in Tenvyr?

In `Tenvyr`, developers write multi-agent pipelines composed of specialized agents (e.g. `code-reviewer`, `observability`, `security`). Designing optimized prompt templates for each agent is difficult.
...
2.  Provide localized `.clauderules` / `.cursorrules` to instruct the developer's local agent on how to write code for Tenvyr itself.
...
This runs the helper script [install-skills.sh](../scripts/install-skills.sh). The script creates a local `skills/` folder containing downloaded instruction templates.
