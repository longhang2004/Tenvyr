...
> **Future roadmap and architectural direction. The current rename does not
...
### Phase 0 — Before merge/public release

| Item                                       | Exit signal                                                                                            |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------ |
| Worker lifecycle and special-key hardening | Shutdown, callback, canonical JSON, stress, and open-handle regressions pass.                          |
| Pack-and-install verification              | Allowlisted tarballs install, compile, run health, stop, and reject deep imports outside the monorepo. |
| Naming and package-scope decision          | Theme J acceptance signal is met before any public package publish.                                    |
| Non-interactive lint setup                 | Repository lint runs without framework prompts and reports a reproducible result.                      |
| Docker/CI validation                       | Supported Docker and CI paths build and run documented checks.                                         |
