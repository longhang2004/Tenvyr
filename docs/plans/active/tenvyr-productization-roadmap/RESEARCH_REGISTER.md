...
| OpenCode                | `opencode run --model provider/model` is the documented model selector; `opencode models [provider]` lists the catalog in `provider/model` format; `opencode models --refresh` refreshes from models.dev; `opencode auth list` lists authenticated providers; `opencode auth login` is the official sign-in. Tenvyr parses bounded CLI output only and NEVER reads `~/.local/share/opencode/auth.json`. Rechecked 2026-08-15. [CLI](https://opencode.ai/docs/cli/), [providers](https://opencode.ai/docs/providers/). |
| 9Router                 | OPTIONAL external router/model source. Tenvyr reads only the OpenAI-compatible `GET {baseUrl}/models` catalog (default candidate `http://localhost:20128/v1`); 9Router owns provider login, OAuth, keys, quota/fallback, and routing. Tenvyr never duplicates that system and never sends inference requests through the source. |
