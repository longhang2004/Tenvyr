...
export class KafkaService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor() {
    ...
    this.consumer = this.kafka.consumer({ groupId: 'agentweave-reviewer-group' });
    ...
    this.consumer.run({
      eachMessage: async (payload: EachMessagePayload) => {
        try {
          ...
          console.error('Error processing code reviewer task:', this.getErrorMessage(err));
        }
      },
    });
  }
  ...
  private async processTask({ message }: EachMessagePayload) {
    ...
    console.log(`Code Reviewer Agent received task for execution [${executionId}], step [${stepId}]`);

    // 1. Read security review skill guidelines from skills/ directory
    let skillRules = 'Analyze code for AWS/Stripe API key secrets, SQL injection (direct interpolation), and missing JWT route guards.';
    try {
      const skillPath = path.resolve(__dirname, '../../../skills/security-review-basic.md');
    ...
    try {
      ...
      try {
        ...
        console.warn('Failed to parse Agent Runner output as JSON. Storing output raw.');
      ...
      console.log(`Publishing Code Reviewer result to agentweave.agent.code-reviewer.result`);
