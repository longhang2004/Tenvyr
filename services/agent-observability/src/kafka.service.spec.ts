...
describe('KafkaService contract boundary', () => {
  ...
  ])('accepts a %s invocation and publishes a v1 result', async (_kind, invocation) => {
  ...
  it('does not crash, call the runner, or publish success for an invalid task', async () => {
    await expect((service as any).processTask(messagePayload({ executionId: 'execution-1' }))).resolves.toBeUndefined();
