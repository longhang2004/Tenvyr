...
export class KafkaService implements OnModuleInit, OnModuleDestroy {
    ...
    this.consumer.run({
      eachMessage: async (payload: EachMessagePayload) => {
        try {
          ...
          console.error(
            "Error processing observability task:",
  ...
  private async processTask({ message }: EachMessagePayload) {
    ...
    console.log(
      `Observability Agent received task for execution [${executionId}], step [${stepId}]`,
    );

    // 1. Read observability guidelines from skills/ directory
    let skillRules =
      "Analyze system log files and identify performance bottlenecks and timeouts.";
    try {
    ...
    try {
      if (!this.isRecord(input))
        throw new Error("Observability input must be an object");
      ...
      try {
        ...
        console.warn(
          "Failed to parse Agent Runner output as JSON. Storing raw.",
        );
        resultData = {
          status: "DEGRADED",
          analysis: runnerOutput,
      ...
      console.log(
        `Publishing Observability result to agentweave.agent.observability.result`,
      );
