...
export class AgentResultService {
  ...
  async handle({ result, transport }: AgentResultMessage): Promise<void> {
    ...
    const stepExecution = await this.executionService.getStepExecutionById(result.stepExecutionId);
    ...
    if (result.invocationId !== expectedInvocationId && legacyAttempt === undefined) {
  ...
  private legacyAttempt(result: AgentResultV1): number | undefined {
    ...
    return this.isRecord(legacy) && typeof legacy.attempt === 'number' ? legacy.attempt : undefined;
  }
