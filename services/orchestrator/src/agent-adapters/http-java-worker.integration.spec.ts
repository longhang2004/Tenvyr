...
const invocation: AgentInvocationV1 = {
  schemaVersion: "1",
  invocationId: "step-execution-java-worker-loopback:1",
  executionId: "execution-java-worker-loopback",
  stepExecutionId: "step-execution-java-worker-loopback",
  stepId: "remote-echo",
  ...
  trace: {
    traceId: "execution-java-worker-loopback",
    correlationId: "step-execution-java-worker-loopback:1",
  },
};

describe("real Orchestrator to Java Worker loopback", () => {
  ...
  beforeAll(async () => {
    const javaExecutable = process.env.TENVYR_JAVA_EXECUTABLE;
    if (!javaExecutable) {
      throw new Error("TENVYR_JAVA_EXECUTABLE is required");
    }
    ...
    workerProcess = spawn(
      javaExecutable,
      ["-cp", javaWorkerClasspath(), "com.tenvyr.worker.OrchestratorLoopbackWorker"],
      {
        env: {
          ...process.env,
    ...
    const ready = await processOutput.waitFor("tenvyr.worker.ready", 15000);
    if (typeof ready.host !== "string" || typeof ready.port !== "number") {
      throw new Error("Java Worker emitted an invalid ready event");
    }
    const workerConfig = parsedConfig.agents.get("remote-echo-agent");
    if (!workerConfig || workerConfig.kind !== "http") {
      throw new Error("Java Worker HTTP configuration is unavailable");
    }
  ...
  afterAll(async () => {
    ...
    if (workerProcess && workerProcess.exitCode === null) {
      ...
      const stopped = await processOutput.waitFor("tenvyr.worker.stopped", 15000);
      expect(stopped.executions).toBe(4);
      const code = await processExit(workerProcess);
      // JVM SIGTERM is 143 unless the fixture intercepts the signal.
      expect([0, 143]).toContain(code);
    }
  });

  it("retries a signed callback and deduplicates without rerunning Java", async () => {
  ...
  it("preserves safe integer boundaries and rejects unsafe Java output", async () => {
  ...
  it("rejects raw unsafe input before reserving its invocation ID", async () => {
    const invocationId = "raw-unsafe-java-worker-loopback:1";
    const rawBody =
...
function javaWorkerClasspath(): string {
  const override = process.env.TENVYR_JAVA_WORKER_CLASSPATH;
  if (override && override.trim()) {
    return override;
  }
  const root = resolve(__dirname, "../../../../sdks/java-worker");
  const classes = join(root, "target/classes");
  const testClasses = join(root, "target/test-classes");
  const depsFile = join(root, "target/loopback-classpath.txt");
  if (!existsSync(classes) || !existsSync(testClasses) || !existsSync(depsFile)) {
    throw new Error(
      "Java Worker loopback classpath is missing; run pretest:java-worker-loopback",
    );
  }
  const deps = readFileSync(depsFile, "utf8").trim();
  if (!deps) {
    throw new Error("Java Worker dependency classpath is empty");
  }
  return [classes, testClasses, deps].join(delimiter);
}
  ...
  async waitFor(eventName: string, timeoutMs = 5000): Promise<LifecycleEvent> {
    ...
    await waitFor(() => {
      ...
      if (this.child.exitCode !== null && !match) {
        throw new Error(
          `Java Worker exited before ${eventName}: ${this.stderr.join("")}`,
        );
      }
