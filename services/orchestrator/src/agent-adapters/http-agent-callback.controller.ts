  ...
  async handle(
    ...
    try {
      ...
      console.warn('HTTP callback rejected', {
        adapter: 'http',
        agent,
        reason: error instanceof AgentAdapterError ? error.code : 'CONTRACT_INVALID',
        deliveryId,
        keyId,
        remoteAddress,
      });

      if (error instanceof ContractValidationError) throw new BadRequestException();
      if (error instanceof AgentAdapterError) {
        if (error.code === 'CALLBACK_UNAUTHORIZED') throw new UnauthorizedException();
        if (error.code === 'CALLBACK_INVALID') throw new BadRequestException();
        if (error.code === 'CALLBACK_HANDLER_UNAVAILABLE') throw new ServiceUnavailableException();
      }
