...
export class HttpAgentAdapter implements AgentAdapter {
  ...
  async start(handler: AgentResultHandler): Promise<void> {
    if (this.resultHandler) {
      ...
      throw new AgentAdapterError('ADAPTER_START_FAILED', this.kind, 'HTTP agent adapter already has a handler', {
        retryable: false,
      });
    }
    ...
    this.replayCleanupTimer = setInterval(() => this.cleanupReplayEntries(Date.now()), replayTtlMs);
  ...
  async invoke(invocation: Parameters<AgentAdapter['invoke']>[0]): Promise<AgentDispatchReceipt> {
    if (!this.resultHandler) {
      throw new AgentAdapterError('ADAPTER_NOT_STARTED', this.kind, 'HTTP agent adapter is not started', {
    ...
    if (!configuration) {
      throw new AgentAdapterError('HTTP_AGENT_NOT_CONFIGURED', this.kind, 'HTTP agent transport is not configured', {
    ...
    try {
      ...
      throw new AgentAdapterError('SERIALIZATION_FAILED', this.kind, 'HTTP invocation could not be serialized', {
        invocationId: payload.invocationId,
        retryable: false,
        cause,
      });
    ...
    try {
      ...
      if (response.status !== 202) {
        throw new AgentAdapterError('HTTP_REJECTED', this.kind, 'Remote HTTP agent rejected the invocation', {
          invocationId: payload.invocationId,
      ...
      const responseBody = await this.readResponse(response, configuration.maxResponseBytes, payload.invocationId);
      ...
      const error =
        cause instanceof AgentAdapterError
          ...
          : new AgentAdapterError(
              ...
              this.kind,
              timedOut ? 'HTTP agent request timed out' : 'HTTP agent connection failed',
              {
                invocationId: payload.invocationId,
                retryable: true,
                cause,
  ...
  async handleCallback(request: HttpCallbackRequest): Promise<'processed' | 'duplicate'> {
    const configuration = this.config.httpForAgent(request.agent);
    if (!configuration || !request.keyId || request.keyId !== configuration.callbackAuthentication.keyId) {
    ...
    if (!this.resultHandler) {
      throw new AgentAdapterError('CALLBACK_HANDLER_UNAVAILABLE', this.kind, 'HTTP callback handler is unavailable', {
        retryable: true,
      });
    }
    ...
    this.reserveReplayEntry(replayKey, now + settings.replayTtlMs, settings.replayMaxEntries);
    ...
    try {
      ...
      throw new AgentAdapterError('CALLBACK_INVALID', this.kind, 'HTTP callback body is invalid JSON', {
        retryable: false,
        cause,
      });
    }
    ...
    try {
      ...
      this.replayEntries.delete(replayKey);
      throw new AgentAdapterError('RESULT_HANDLER_FAILED', this.kind, 'HTTP callback result handler failed', {
  ...
  private async readResponse(response: Response, limit: number, invocationId: string): Promise<string> {
    const contentLength = Number(response.headers.get('content-length'));
    if (Number.isFinite(contentLength) && contentLength > limit) throw responseTooLarge(invocationId);
  ...
  private reserveReplayEntry(key: string, expiresAt: number, maxEntries: number): void {
...
function invalidResponse(invocationId: string, cause?: unknown): AgentAdapterError {
  return new AgentAdapterError('HTTP_INVALID_RESPONSE', 'http', 'Remote HTTP agent returned an invalid response', {
    invocationId,
    retryable: false,
    cause,
  });
...
function responseTooLarge(invocationId: string): AgentAdapterError {
  return new AgentAdapterError('HTTP_RESPONSE_TOO_LARGE', 'http', 'Remote HTTP agent response exceeded the limit', {
    invocationId,
    retryable: false,
  });
}
