...
describe('HTTP agent loopback integration', () => {
  ...
  let submittedRequest: { method?: string; headers: Record<string, string | string[] | undefined>; body: any };
  ...
  beforeAll(async () => {
    ...
    const module = await Test.createTestingModule({
      ...
      providers: [{ provide: AgentTransportConfigService, useValue: config }, HttpAgentAdapter],
    }).compile();
  ...
  it('submits to a real local agent and receives a signed callback through Nest', async () => {
    ...
    const response = await fetch(submittedRequest.body.resultDelivery.callbackUrl, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
