...
export type AgentTransportConfiguration = KafkaAgentTransportConfiguration | HttpAgentTransportConfiguration;
...
export function parseAgentTransportConfiguration(
  ...
  try {
    ...
    throw invalidConfiguration('AGENT_TRANSPORT_CONFIG must be valid JSON', cause);
  }
  if (!isRecord(value)) throw invalidConfiguration('AGENT_TRANSPORT_CONFIG must be an object');
  ...
  for (const [agent, entry] of Object.entries(value)) {
    if (!agent || !isRecord(entry)) throw invalidConfiguration('Every agent configuration must be an object');
    if (entry.kind === 'kafka') {
      assertOnlyKeys(entry, ['kind'], agent);
      agents.set(agent, { kind: 'kafka' });
      continue;
    }
    if (entry.kind !== 'http') throw invalidConfiguration(`Agent "${agent}" has an unsupported transport kind`);

    assertOnlyKeys(
      entry,
      ['kind', 'submitUrl', 'outboundAuthentication', 'callbackAuthentication', 'requestTimeoutMs', 'maxResponseBytes'],
      agent,
    );
    const submitUrl = validatedUrl(entry.submitUrl, `Agent "${agent}" submit URL`, allowInsecure);
    const outboundAuthentication = parseOutboundAuthentication(entry.outboundAuthentication, environment, agent);
    const callbackAuthentication = parseCallbackAuthentication(entry.callbackAuthentication, environment, agent);
    const requestTimeoutMs = positiveInteger(entry.requestTimeoutMs, `Agent "${agent}" request timeout`);
    const maxResponseBytes = positiveInteger(entry.maxResponseBytes, `Agent "${agent}" response-size limit`);
  ...
  const hasHttpAgent = Array.from(agents.values()).some((configuration) => configuration.kind === 'http');
  const callbackBaseUrl = hasHttpAgent
    ? validatedUrl(environment.HTTP_AGENT_CALLBACK_BASE_URL, 'HTTP_AGENT_CALLBACK_BASE_URL', allowInsecure)
    : undefined;
  ...
  if (replayTtlMs < callbackMaxSkewSeconds * 1000) {
    throw invalidConfiguration('HTTP_AGENT_REPLAY_TTL_MS must cover the callback clock-skew window');
  }
...
export class AgentTransportConfigService {
  ...
  callbackUrlFor(agent: string): string {
    if (!this.configuration.callbackBaseUrl) {
      throw invalidConfiguration('HTTP_AGENT_CALLBACK_BASE_URL is required for HTTP agents');
    }
...
function parseOutboundAuthentication(
  ...
  if (!isRecord(value)) throw invalidConfiguration(`Agent "${agent}" outbound authentication is required`);
  if (value.type === 'none') {
    assertOnlyKeys(value, ['type'], agent);
    return { type: 'none' };
  }
  if (value.type !== 'bearer') {
    throw invalidConfiguration(`Agent "${agent}" has unsupported outbound authentication`);
  }
  assertOnlyKeys(value, ['type', 'tokenEnv'], agent);
  const tokenEnv = nonEmptyString(value.tokenEnv, `Agent "${agent}" bearer token environment name`);
  const token = environment[tokenEnv];
  if (!token) throw invalidConfiguration(`Agent "${agent}" bearer token environment value is missing`);
...
function parseCallbackAuthentication(
  ...
  if (!isRecord(value)) throw invalidConfiguration(`Agent "${agent}" callback authentication is required`);
  assertOnlyKeys(value, ['keyId', 'secretEnv'], agent);
  const keyId = nonEmptyString(value.keyId, `Agent "${agent}" callback key ID`);
  const secretEnv = nonEmptyString(value.secretEnv, `Agent "${agent}" callback secret environment name`);
  const secret = environment[secretEnv];
  if (!secret) throw invalidConfiguration(`Agent "${agent}" callback secret environment value is missing`);
  return { keyId, secret };
}

function validatedUrl(value: unknown, field: string, allowInsecure: boolean): string {
  ...
  if (url.username || url.password) throw invalidConfiguration(`${field} must not contain credentials`);
  if (url.protocol !== 'https:' && url.protocol !== 'http:') {
    throw invalidConfiguration(`${field} must use http or https`);
  }
  if (url.protocol === 'http:' && !allowInsecure) {
    throw invalidConfiguration(`${field} requires HTTPS unless HTTP_AGENT_ALLOW_INSECURE=true`);
  }
  return url.toString();
}
...
function nonEmptyString(value: unknown, field: string): string {
  if (typeof value !== 'string' || !value.trim()) throw invalidConfiguration(`${field} must be a non-empty string`);
  return value;
}

function assertOnlyKeys(value: Record<string, unknown>, allowed: string[], agent: string): void {
  if (Object.keys(value).some((key) => !allowed.includes(key))) {
    throw invalidConfiguration(`Agent "${agent}" configuration contains an unsupported field`);
  }
}
...
function invalidConfiguration(message: string, cause?: unknown): AgentAdapterError {
  return new AgentAdapterError('HTTP_CONFIGURATION_INVALID', 'router', message, {
    retryable: false,
    cause,
  });
}
