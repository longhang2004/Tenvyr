...
import { ContractValidationError } from '@agentweave/contracts';
...
describe('HttpAgentCallbackController', () => {
  ...
  it.each([
    [
      ...
      new AgentAdapterError('CALLBACK_UNAUTHORIZED', 'http', 'unauthorized', { retryable: false }),
      401,
    ],
    ['invalid JSON', new AgentAdapterError('CALLBACK_INVALID', 'http', 'invalid callback', { retryable: false }), 400],
    ['invalid result contract', new ContractValidationError('AgentResultV1', [{ path: '/', message: 'invalid' }]), 400],
    [
      'unavailable handler',
      new AgentAdapterError('CALLBACK_HANDLER_UNAVAILABLE', 'http', 'unavailable', { retryable: true }),
      503,
    ],
    [
      'handler failure',
      new AgentAdapterError('RESULT_HANDLER_FAILED', 'http', 'handler failed', { retryable: true }),
      500,
    ],
