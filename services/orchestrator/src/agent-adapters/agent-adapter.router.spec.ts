...
describe('AgentAdapterRouter', () => {
  ...
  it('routes exact HTTP agents to HTTP and preserves the receipt', async () => {
    ...
    expect(receipt).toMatchObject({ adapter: 'http', dispatchId: 'remote-run-1' });
  });

  it.each(['code-reviewer', 'observability', 'unknown'])('preserves Kafka routing for %s', async (agent) => {
  ...
  ])('does not fallback when %s dispatch fails', async (_case, agent, failing, other) => {
  ...
  it('cleans up Kafka when HTTP startup fails', async () => {
    ...
    await expect(router.start(handler)).rejects.toMatchObject({ code: 'ADAPTER_START_FAILED' });
  ...
  it('best-effort stops the second adapter when the first stop fails', async () => {
    ...
    await expect(router.stop()).rejects.toMatchObject({ code: 'ADAPTER_STOP_FAILED' });
    expect(kafka.stop).toHaveBeenCalledTimes(1);
  });

  it('rejects invoke before the router has started', async () => {
    await expect(router.invoke(invocation('code-reviewer'))).rejects.toMatchObject({
