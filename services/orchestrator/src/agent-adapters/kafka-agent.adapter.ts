...
export class KafkaAgentAdapter implements AgentAdapter {
  ...
  async start(handler: AgentResultHandler): Promise<void> {
    ...
    try {
      ...
      connected = true;
      await this.kafka.subscribe(resultTopics, async (payload) => this.handleKafkaMessage(payload));
      this.started = true;
      ...
      throw new AgentAdapterError('ADAPTER_START_FAILED', this.kind, 'Kafka agent adapter failed to start', {
        retryable: true,
        cause,
      });
    }
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      throw new AgentAdapterError('ADAPTER_STOP_FAILED', this.kind, 'Kafka agent adapter failed to stop', {
        retryable: true,
        cause,
      });
    }
  ...
  async invoke(invocation: Parameters<AgentAdapter['invoke']>[0]): Promise<AgentDispatchReceipt> {
    if (!this.started) {
      throw new AgentAdapterError('ADAPTER_NOT_STARTED', this.kind, 'Kafka agent adapter is not started', {
    ...
    try {
      ...
      throw new AgentAdapterError('SERIALIZATION_FAILED', this.kind, 'Agent invocation could not be serialized', {
    ...
    try {
      ...
      const error = new AgentAdapterError('DISPATCH_FAILED', this.kind, 'Kafka agent invocation dispatch failed', {
        invocationId: payload.invocationId,
        retryable: true,
        cause,
      });
  ...
  private async handleKafkaMessage({ topic, partition, message }: EachMessagePayload): Promise<void> {
    ...
    try {
      ...
      if (!this.resultHandler) {
        throw new AgentAdapterError('RESULT_HANDLER_FAILED', this.kind, 'Agent result handler is unavailable', {
          invocationId: result.invocationId,
          retryable: true,
        });
      }
      ...
      try {
        ...
        throw new AgentAdapterError('RESULT_HANDLER_FAILED', this.kind, 'Agent result handler failed', {
          invocationId: result.invocationId,
          retryable: true,
          cause,
        });
  ...
  private async normalizeResult(value: unknown, kafkaTimestamp: string): Promise<AgentResultV1 | null> {
    ...
    const stepExecution = await this.executionService.getStepExecution(executionId, stepId);
    if (!stepExecution) {
    ...
    const attempt = this.isRecord(value) && typeof value.attempt === 'number' ? value.attempt : stepExecution.attempt;
    const completedAt =
      (this.isRecord(value) && typeof value.timestamp === 'string' && value.timestamp) ||
  ...
  private resultTopics(): string[] {
    const agents = (process.env.ORCHESTRATOR_AGENT_NAMES || 'code-reviewer,observability')
      .split(',')
    ...
    return Array.from(new Set([...agents.map((agent) => `agentweave.agent.${agent}.result`), ...explicitTopics]));
  }
  ...
  private logInboundError(error: unknown, transport: AgentTransportMetadata, payload: unknown): void {
    const context = {
      adapter: this.kind,
