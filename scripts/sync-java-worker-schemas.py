...
DESTINATION = (
    REPO_ROOT / "sdks" / "java-worker" / "src" / "main" / "resources" / "schema_json"
)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Synchronize the tracked Java Worker schema resources"
    )
...
def check() -> int:
    ...
    if mismatches:
        print("Java Worker schema resources are out of sync:")
        for filename in sorted(set(mismatches)):
            print(f"- {filename}")
        return 1
    print("Java Worker schema resources are in sync (5/5).")
    return 0
